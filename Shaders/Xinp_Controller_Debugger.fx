...
//uniform float gamepad_toggle[25] < source = "gamepad_toggle"; >;
//uniform float gamepad_toggle_raw[25]    < source = "gamepad_raw";    >;
uniform float2 gamepad_toggle_raw[25] < source = "gamepad_toggle_raw"; > ;//was added because of DX9
...
float4 DrawButtons(float2 tc, float2 resolution)
{
    ...
    //Guide button centered between Start and Select, slightly above
    float2 posGuide = center + float2(0.0 * aspect, -0.10 + ABXYS_off);
    ...
    float guAlpha = DrawCircle(p, posGuide, btnRadius * 1.5, feather, aspect);
    ...
    float4 colorGU = float4(0.25, 0.25, 0.25, 1.0);
    ...
    colorGU.rgb = lerp(colorGU.rgb, 1, gamepad_toggle_raw[20].y);
...
//gamepad_toggle_raw[24].x is Back + Right Bump
//gamepad_toggle_raw[23].x is Back + Right Trigger
//gamepad_toggle_raw[22].x is Back + Left Bump
//gamepad_toggle_raw[21].x is Back + Left Trigger
//gamepad_toggle_raw[20].x is Guide Button
}
